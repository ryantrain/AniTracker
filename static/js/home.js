const myInput = document.getElementById("search-query");
const storageDisplay = document.getElementsByClassName("search-bar-dropdown");
const body = document.body;
const navToggle = document.getElementById('nav-toggle');
const navBackdrop = document.querySelector('.nav-backdrop');
const profileIcon = document.getElementById('profile-icon');
const profileBackdrop = document.querySelector('.profile-icon-backdrop');
const historyIcon = '<svg viewBox="0 0 24 24" height="24px" width="24px" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M3 5.67541V3C3 2.44772 2.55228 2 2 2C1.44772 2 1 2.44772 1 3V7C1 8.10457 1.89543 9 3 9H7C7.55229 9 8 8.55229 8 8C8 7.44772 7.55229 7 7 7H4.52186C4.54218 6.97505 4.56157 6.94914 4.57995 6.92229C5.621 5.40094 7.11009 4.22911 8.85191 3.57803C10.9074 2.80968 13.173 2.8196 15.2217 3.6059C17.2704 4.3922 18.9608 5.90061 19.9745 7.8469C20.9881 9.79319 21.2549 12.043 20.7247 14.1724C20.1945 16.3018 18.9039 18.1638 17.0959 19.4075C15.288 20.6513 13.0876 21.1909 10.9094 20.9247C8.73119 20.6586 6.72551 19.605 5.27028 17.9625C4.03713 16.5706 3.27139 14.8374 3.06527 13.0055C3.00352 12.4566 2.55674 12.0079 2.00446 12.0084C1.45217 12.0088 0.995668 12.4579 1.04626 13.0078C1.25994 15.3309 2.2082 17.5356 3.76666 19.2946C5.54703 21.3041 8.00084 22.5931 10.6657 22.9188C13.3306 23.2444 16.0226 22.5842 18.2345 21.0626C20.4464 19.541 22.0254 17.263 22.6741 14.6578C23.3228 12.0526 22.9963 9.30013 21.7562 6.91897C20.5161 4.53782 18.448 2.69239 15.9415 1.73041C13.4351 0.768419 10.6633 0.756291 8.14853 1.69631C6.06062 2.47676 4.26953 3.86881 3 5.67541Z" fill="#FFFFFF"></path> <path d="M12 5C11.4477 5 11 5.44771 11 6V12.4667C11 12.4667 11 12.7274 11.1267 12.9235C11.2115 13.0898 11.3437 13.2344 11.5174 13.3346L16.1372 16.0019C16.6155 16.278 17.2271 16.1141 17.5032 15.6358C17.7793 15.1575 17.6155 14.546 17.1372 14.2698L13 11.8812V6C13 5.44772 12.5523 5 12 5Z" fill="#FFFFFF"></path> </g></svg>';

myInput.addEventListener("keydown", function(event) {
    if (event.key === "Enter"){
        event.preventDefault();

        const storedInput = Object.entries(localStorage)
        const userInput = myInput.value;

        localStorage.setItem("userInput"+ (storedInput.length+1) , userInput)

        window.location.href = "/search/" + userInput + "/";
    }
});

document.querySelector(".search-bar-backdrop").addEventListener("pointerdown", function(event) {
    event.preventDefault();
    myInput.value = '';
    myInput.blur();
    body.classList.remove('search-active');
});

// Populate dropdown from localStorage
function renderStorageDropdown() {
    storageDisplay.innerHTML = '';

    if (localStorage.length === 0) {
        const div = document.createElement('a');
        div.innerHTML = 'No recent searches';
        div.classList.add('search-bar-dropdown-item-empty');
        storageDisplay[0].appendChild(div);
    } else {
        if (document.getElementsByClassName("search-bar-dropdown-item-empty").length > 0) {
            document.getElementsByClassName("search-bar-dropdown-item-empty")[0].remove();
        }
        for (let i = localStorage.length; i > 0; i--) {
            const key = localStorage.key(i);
            const value = localStorage.getItem("userInput"+i);
            if (value && value.trim() !== ''){
                const div = document.createElement('a');
                div.classList.add('search-bar-dropdown-item');
                div.setAttribute('href', './search/' + value + '/');
                div.innerHTML = historyIcon + value;
                div.addEventListener('click', () => {
                    storageDisplay.innerHTML = '';  
                    window.location.href = "/search/" + value + "/";
                });
                storageDisplay[0].appendChild(div);
            }

        }

    }
}

renderStorageDropdown();

// show the global backdrop while the input is focused
myInput.addEventListener('focus', function() {
    document.body.style.overflow = 'hidden';
    if (!navToggle.checked){
        body.classList.add('search-active');
    }
    
});

navToggle.addEventListener('change', function(){
    if (navToggle.checked) {
        body.classList.add('nav-active');
    } else {
        body.classList.remove('nav-active');
    }
})

if (navBackdrop) {
    navBackdrop.addEventListener('pointerdown', function(event) {
        event.preventDefault();
        navToggle.checked = false;
        body.classList.remove('nav-active');
    });
}

myInput.addEventListener('blur', function() {
    document.body.style.overflow = 'auto';

    setTimeout(() => {
        if (document.activeElement !== myInput) {
            body.classList.remove('search-active');
        }
    }, 0);
});

if (profileIcon) {
    profileIcon.addEventListener('click', function(event) {
        event.preventDefault();
        event.stopPropagation();
        body.classList.toggle('profile-active');
    });
}

if (profileBackdrop) {
    profileBackdrop.addEventListener('pointerdown', function(event) {
        event.preventDefault();
        body.classList.remove('profile-active');
    });
}

const featuredSliderViewport = document.querySelector('[data-featured-slider]');

if (featuredSliderViewport && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    const featuredSliderTrack = featuredSliderViewport.querySelector('.featured-slider-track');
    const originalFeaturedCards = Array.from(featuredSliderTrack.querySelectorAll('.featured-card'));
    let featuredCards = [];
    let featuredSliderTimer = null;
    let featuredSliderIndex = 0;
    let isPaused = false;

    let cachedOffsets = [];
    const cacheVisibleCards = () => {
        featuredSliderTrack.querySelectorAll('.featured-card[data-clone="true"]').forEach((card) => card.remove());

        originalFeaturedCards.forEach((card) => {
            const clone = card.cloneNode(true);
            clone.setAttribute('data-clone', 'true');
            clone.setAttribute('aria-hidden', 'true');
            clone.setAttribute('tabindex', '-1');
            featuredSliderTrack.appendChild(clone);
        });

        featuredCards = Array.from(featuredSliderTrack.querySelectorAll('.featured-card'));
    };

    const cacheOffsets = () => {
        cachedOffsets = featuredCards.map(card => card.offsetLeft);
    };

    cacheVisibleCards();
    cacheOffsets();

    const getVisibleCardCount = () => {
        const firstCard = originalFeaturedCards[0];
        if (!firstCard || !featuredSliderTrack) return 1;
        const gap = parseFloat(getComputedStyle(featuredSliderTrack).gap) || 0;
        const step = firstCard.getBoundingClientRect().width + gap;
        return Math.max(1, Math.floor((featuredSliderViewport.clientWidth + gap) / step));
    };

    const getLastStartIndex = () => Math.max(0, featuredCards.length - getVisibleCardCount());

    const scrollToIndex = (index, behavior = 'smooth') => {
        const left = cachedOffsets[index];
        if (left == null) return;
        featuredSliderViewport.scrollTo({ left, behavior });
    };

    const SLIDE_INTERVAL = 2000;

    const schedule = (delay = SLIDE_INTERVAL) => {
        clearTimeout(featuredSliderTimer);
        featuredSliderTimer = setTimeout(tick, delay);
    };

    function tick() {
        featuredSliderTimer = null;
        if (isPaused) return;

        const lastStartIndex = getLastStartIndex();

        if (featuredSliderIndex >= lastStartIndex) {
            featuredSliderIndex = 0;
            scrollToIndex(0, 'auto');
            schedule(SLIDE_INTERVAL);
            return;
        }

        featuredSliderIndex += 1;
        scrollToIndex(featuredSliderIndex, 'smooth');
        schedule(SLIDE_INTERVAL);
    }

    const startFeaturedSlider = () => {
        isPaused = false;
        if (featuredCards.length <= 1) return;
        if (featuredSliderTimer === null) schedule(SLIDE_INTERVAL);
    };

    const stopFeaturedSlider = () => {
        isPaused = true;
        clearTimeout(featuredSliderTimer);
        featuredSliderTimer = null;
    };

    featuredSliderViewport.addEventListener('mouseenter', stopFeaturedSlider);
    featuredSliderViewport.addEventListener('mouseleave', startFeaturedSlider);
    featuredSliderViewport.addEventListener('focusin', stopFeaturedSlider);
    featuredSliderViewport.addEventListener('focusout', startFeaturedSlider);

    window.addEventListener('resize', () => {
        cacheVisibleCards();
        cacheOffsets();
        featuredSliderIndex = Math.min(featuredSliderIndex, getLastStartIndex());
        scrollToIndex(featuredSliderIndex, 'auto');
    });

    startFeaturedSlider();
}