...
// Ensure the left column width matches the rendered thumbnail width
function syncLeftColumnToImage() {
    const img = document.querySelector('.anime-thumbnail img');
    const left = document.querySelector('.left-column');
    if (!img || !left) return;

    const applyWidth = () => {
        // Use the image's layout width (rounded) so text wraps to it
    ...
    // If image has already loaded, apply immediately
    if (img.complete) applyWidth();
    // Also update when image finishes loading
...
document.addEventListener('DOMContentLoaded', syncLeftColumnToImage);
